...
function saveGame() {
  _persistVideoStreak(); // guardar racha de videos
  G.lastSave = Date.now();
...
// ═══════════════════════════════════════════════════════════════════
// SISTEMA DE VIDEOS YOUTUBE CON RACHA DE GEMAS
// ═══════════════════════════════════════════════════════════════════

// Estado del sistema de videos
let _videoActive    = false;
let _videoStreak    = 0;        // racha actual (0-based, resetea con cooldown)
let _videoStreakTs  = 0;        // timestamp del último video de la racha
let _videoYTReady   = false;    // YouTube IFrame API cargada
let _videoYTPlayer  = null;     // instancia del player
let _videoWatchIv   = null;     // interval de verificación de tiempo visto
let _videoSecsWatched = 0;      // segundos reales viendo el video
let _videoCurrentData = null;   // metadata del video actual

// Racha: gemas según posición
function _videoStreakGems(streak) {
  // streak 0=primer video, 1=segundo, 2=tercer en adelante
  if (streak === 0) return 3;
  if (streak === 1) return 5;
  return 8;
}

// Cooldown: 1 hora entre sesiones de video (no entre videos de la misma racha)
const _VIDEO_COOLDOWN_MS  = 60 * 60 * 1000;  // 1 hora
...
async function _loadVideoPool() {
  ...
  try {
    ...
    if (!error && data && data.length > 0) {
      // Expandir por peso para rotación ponderada
      _videoPool = [];
      data.forEach(v => {
        const w = v.weight || 10;
        for (let i = 0; i < Math.ceil(w / 5); i++) _videoPool.push(v);
      });
    }
  } catch(e) {}
}

// Seleccionar video aleatorio (ponderado si viene de Supabase)
function _pickRandomVideo() {
  return _videoPool[Math.floor(Math.random() * _videoPool.length)];
}

// Cargar YouTube IFrame API una sola vez
function _loadYouTubeAPI(cb) {
  if (_videoYTReady) { cb(); return; }
  if (document.getElementById('yt-api-script')) {
    // Ya está cargando — esperar
    const wait = setInterval(() => { if (_videoYTReady) { clearInterval(wait); cb(); } }, 200);
    return;
  }
  window.onYouTubeIframeAPIReady = () => { _videoYTReady = true; cb(); };
...
// Entrada principal — llamada desde botón "Ver video"
function watchAd() {
  if (_videoActive) return;

  const now = Date.now();
  const cooldown = _VIDEO_COOLDOWN_MS;

  // Calcular racha: se mantiene si el último video fue hace menos de 10 min
  if ((now - _videoStreakTs) > _VIDEO_STREAK_WINDOW && _videoStreakTs > 0) {
    _videoStreak = 0; // racha expiró
  }

  // Cooldown de 1 hora desde el último video (solo si racha ya completó 3 o más)
  if (_videoStreak === 0 && (now - (G.lastAdTs || 0)) < cooldown) {
    const remaining = Math.ceil((cooldown - (now - G.lastAdTs)) / 60000);
    const mins = remaining >= 60
      ? `${Math.floor(remaining/60)}h ${remaining%60}min`
      : `${remaining} min`;
    notify(`⏳ Próximo video en ${mins}`);
    return;
  }
...
function _showVideoOverlay(video) {
  // Quitar overlay previo si existe
  document.getElementById('videoAdOverlay')?.remove();

  const gemsNext = _videoStreakGems(_videoStreak);
  const streakLabel = _videoStreak === 0 ? '' :
    _videoStreak === 1 ? '🔥 Racha x2 — ' : '🔥🔥 Racha x3 — ';
  ...
  overlay.innerHTML = `
    ...
    <div style="width:100%;max-width:560px;display:flex;align-items:center;justify-content:space-between;
      padding:10px 14px;background:#111;border-bottom:2px solid #222;flex-shrink:0">
      <div>
        <div style="font-size:0.6rem;color:#888;font-weight:900;text-transform:uppercase;letter-spacing:1.5px">
          ${video.sponsor_name ? `✨ Patrocinado por ${video.sponsor_name}` : '🎬 Video del Barrio'}
        </div>
        <div style="font-family:'Fredoka One',cursive;font-size:0.95rem;color:white;margin-top:1px;
          max-width:220px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">
          ${video.title}
        </div>
      </div>
      <div style="text-align:right">
        <div style="font-size:0.58rem;color:#888;font-weight:900">${streakLabel}Ganas</div>
        <div style="font-family:'Fredoka One',cursive;font-size:1.3rem;color:#D4A8FF">+${gemsNext} 💎</div>
      </div>
    </div>

    <!-- Player container -->
    <div id="ytPlayerContainer" style="width:100%;max-width:560px;flex:1;background:#000;position:relative;min-height:200px">
      <div id="ytPlayer"></div>
    </div>

    <!-- Footer: barra de progreso + racha + botón -->
    <div style="width:100%;max-width:560px;background:#111;border-top:2px solid #222;
      padding:12px 14px 16px;flex-shrink:0">

      <!-- Racha dots -->
      <div style="display:flex;align-items:center;gap:6px;margin-bottom:8px">
        <div style="font-size:0.6rem;color:#888;font-weight:900;text-transform:uppercase;letter-spacing:1px">Racha</div>
        ${[0,1,2].map(i => `
          <div style="width:28px;height:28px;border-radius:50%;border:2px solid ${i <= _videoStreak ? '#9B5DE5' : '#333'};
            background:${i < _videoStreak ? '#9B5DE5' : i === _videoStreak ? 'rgba(155,93,229,0.2)' : 'transparent'};
            display:flex;align-items:center;justify-content:center;font-size:0.65rem;font-weight:900;
            color:${i < _videoStreak ? 'white' : i === _videoStreak ? '#D4A8FF' : '#555'}">
            ${i < _videoStreak ? '✓' : [3,5,8][i] + '💎'}
          </div>`).join('')}
        <div style="margin-left:4px;font-size:0.58rem;color:#666;font-weight:700">
          ${_videoStreak < 2 ? `+${_videoStreakGems(_videoStreak+1)} 💎 en el siguiente` : '¡Máximo alcanzado!'}
        </div>
      </div>

      <!-- Barra de progreso -->
      <div style="background:rgba(255,255,255,0.08);border-radius:99px;height:7px;overflow:hidden;margin-bottom:10px;position:relative">
        <div id="videoProgressBar" style="height:100%;width:0%;background:linear-gradient(90deg,#7B2FBE,#D4A8FF);
          border-radius:99px;transition:width 1s linear"></div>
      </div>

      <div style="display:flex;align-items:center;justify-content:space-between">
        <div style="font-size:0.7rem;color:#666;font-weight:900">
          <span id="videoSecsLeft">${video.min_watch_sec}</span>s para desbloquear
        </div>
        <button id="videoCancelBtn" onclick="_cancelVideo()"
          style="background:rgba(255,255,255,0.06);color:rgba(255,255,255,0.4);
            border:1.5px solid rgba(255,255,255,0.1);border-radius:99px;
            font-family:'Fredoka One',cursive;font-size:0.75rem;padding:6px 16px;cursor:pointer">
          Cancelar
        </button>
      </div>
    </div>
  ...
  // Ajustar el player al tamaño disponible
  const container = document.getElementById('ytPlayerContainer');
  const playerEl  = document.getElementById('ytPlayer');
  const h = container.clientHeight || Math.round(window.innerWidth * 0.5625);

  // Crear el player de YouTube
  _videoYTPlayer = new YT.Player(playerEl, {
    height: h,
    width:  '100%',
    videoId: video.youtube_id,
    playerVars: {
      autoplay:       1,
      controls:       1,    // mostrar controles pero no se puede saltar
      rel:            0,
      modestbranding: 1,
      playsinline:    1,
      fs:             0,    // sin fullscreen para evitar evasión
    },
    events: {
      onReady:       _onYTReady,
...
function _onYTStateChange(e) {
  // Si el usuario pausó, pausar el contador también
  if (e.data === YT.PlayerState.PLAYING)  _startWatchTimer();
  if (e.data === YT.PlayerState.PAUSED)   _pauseWatchTimer();
  if (e.data === YT.PlayerState.ENDED)    _onVideoEnded();
  if (e.data === YT.PlayerState.BUFFERING) {} // no hacer nada
}

function _startWatchTimer() {
  if (_videoWatchIv) return; // ya corriendo
  const minSecs = _videoCurrentData?.min_watch_sec || 15;
  _videoWatchIv = setInterval(() => {
    _videoSecsWatched++;
    const pct = Math.min(100, (_videoSecsWatched / minSecs) * 100);
    const secsLeft = Math.max(0, minSecs - _videoSecsWatched);
    const bar = document.getElementById('videoProgressBar');
    const lbl = document.getElementById('videoSecsLeft');
    if (bar) bar.style.width = pct + '%';
    if (lbl) lbl.textContent = secsLeft;

    if (_videoSecsWatched >= minSecs) {
      _pauseWatchTimer();
      _onWatchComplete();
    }
  }, 1000);
...
function _pauseWatchTimer() {
  if (_videoWatchIv) { clearInterval(_videoWatchIv); _videoWatchIv = null; }
}

function _onVideoEnded() {
  _pauseWatchTimer();
  if (_videoSecsWatched >= (_videoCurrentData?.min_watch_sec || 15)) {
    _onWatchComplete();
  } else {
    // Terminó antes de tiempo — no dar gemas pero tampoco cancelar
    notify('⚠️ El video terminó muy pronto — prueba otro');
    _cleanupVideoOverlay();
  }
}

function _onWatchComplete() {
  const gems = _videoStreakGems(_videoStreak);
  const isMaxStreak = _videoStreak >= 2;

  // Dar gemas
  addGems(gems);

  // Actualizar estado
  _videoStreak    = Math.min(_videoStreak + 1, 3);
  _videoStreakTs  = Date.now();
  if (isMaxStreak || _videoStreak >= 3) {
    // Racha completada: iniciar cooldown de 1 hora
    G.lastAdTs   = Date.now();
    _videoStreak = 0;
  }

  // Log en Supabase si tiene cuenta
  _logVideoView(gems);

  saveGame();

  // Mostrar celebración antes de cerrar
  _showVideoReward(gems, isMaxStreak);
}

function _showVideoReward(gems, isMax) {
  _cleanupVideoPlayer();
  ...
  overlay.innerHTML = `
    <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;
      height:100%;gap:16px;padding:24px;text-align:center;animation:fadeInMs 0.4s ease">
      <div style="font-size:4rem;animation:pulseMs 0.8s ease infinite">🎬</div>
      <div style="font-family:'Fredoka One',cursive;font-size:1.8rem;color:#D4A8FF">
        ${isMax ? '🔥 ¡Racha Completa!' : '¡Gracias por ver!'}
      </div>
      <div style="font-family:'Fredoka One',cursive;font-size:3rem;color:#FFE135">
        +${gems} 💎
      </div>
      <div style="font-size:0.8rem;color:rgba(255,255,255,0.5);font-weight:700">
        ${isMax
          ? 'Próxima sesión de videos en 1 hora'
          : `Ver ${3 - Math.min(_videoStreak, 3) + (_videoStreak > 0 ? 0 : 0)} video${_videoStreak > 0 ? '' : 's'} más para racha completa`
        }
      </div>
      <button onclick="_closeVideoOverlay()"
        style="background:linear-gradient(135deg,#7B2FBE,#9B5DE5);color:white;
          font-family:'Fredoka One',cursive;font-size:1.1rem;padding:14px 36px;
          border:3px solid white;border-radius:99px;cursor:pointer;
          box-shadow:0 6px 20px rgba(123,47,190,0.5);margin-top:8px">
        ¡Genial! 🎉
      </button>
    </div>
  `;
  ...
  for (let i = 0; i < 30; i++) {
    const c = document.createElement('div');
    c.style.cssText = `position:fixed;z-index:9999;pointer-events:none;border-radius:50%;
      width:${5+Math.random()*6}px;height:${5+Math.random()*6}px;
      background:${['#D4A8FF','#FFE135','#2DC653','#FF6348'][Math.floor(Math.random()*4)]};
  ...
  notify(`🎬 +${gems} 💎 ¡Video completado!`);
  renderGemSources();
}

function _cancelVideo() {
  _pauseWatchTimer();
  notify('❌ Video cancelado — no se ganaron gemas');
  _closeVideoOverlay();
}
...
function _cleanupVideoPlayer() {
  _pauseWatchTimer();
  try { _videoYTPlayer?.destroy(); } catch(e) {}
  _videoYTPlayer = null;
}

function _cleanupVideoOverlay() {
  document.getElementById('videoAdOverlay')?.remove();
}
...
async function _logVideoView(gems) {
  ...
  try {
    await sb.from('video_views').insert({
      ...
      gems_earned: gems,
      streak_day:  _videoStreak,
    });
  } catch(e) {}
}

// Cargar el pool de videos cuando el cloud esté listo
// (se llama desde el initCloud existente)
function _initVideoSystem() {
  // Restaurar racha desde G si existe
  if (G._videoStreak)   _videoStreak  = G._videoStreak;
  if (G._videoStreakTs) _videoStreakTs = G._videoStreakTs;
  // Expirar racha si pasaron más de 10 min
  if ((Date.now() - _videoStreakTs) > _VIDEO_STREAK_WINDOW) _videoStreak = 0;
  _loadVideoPool();
}

// Persistir racha en G para que sobreviva recargas
const _origSaveGame = typeof saveGame !== 'undefined' ? saveGame : null;
function _persistVideoStreak() {
  G._videoStreak  = _videoStreak;
  G._videoStreakTs = _videoStreakTs;
}
...
function renderGemSources() {
  ...
  const instant = [
    ...
    },
    {
      ...
      name: 'Ver un Video',
      desc: (() => {
        const streakNow = _videoStreak || 0;
        const gemsNow   = _videoStreakGems(streakNow);
        if (streakNow === 0) return `Ver un video de YouTube · Gana ${gemsNow} 💎`;
        if (streakNow === 1) return `🔥 Racha x2 · ¡Siguiente video vale ${gemsNow} 💎!`;
        return `🔥🔥 Racha x3 · ¡Último video vale ${gemsNow} 💎!`;
      })(),
      gems: (() => {
        const s = _videoStreak || 0;
        return `+${_videoStreakGems(s)} 💎`;
      })(),
      id: 'ad',
      cooldown: _VIDEO_COOLDOWN_MS,
      last: (_videoStreak > 0 && (Date.now() - _videoStreakTs) < _VIDEO_STREAK_WINDOW)
        ? 0  // hay racha activa — mostrar disponible
        : G.lastAdTs || 0,
...
// Tasas: 💎→💰 10💎=$5M | 💰→💎 $500K=1💎 | 💠 1 influencia=1000💎
// ═══════════════════════════════════════════════════════════
...
runSplash(() => {
  ...
  if (isRealSave) {
    ...
    setTimeout(() => _initVideoSystem(), 2000); // cargar pool de videos
  } else {
